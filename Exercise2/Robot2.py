...
class Robot:
    ...
    def __init__(self, **kwargs):
        ...
        self._braitenberg = False  # Define if robot is a braitenberg vehicle
        ...
        # If robot is a braitenberg vehicle (braitenberg == True), set sensor amount to 2
        if 'braitenberg' in kwargs:
            if kwargs['braitenberg']:
                self._braitenberg = True
                self._numberOfSensors = 2
                self._sensorDirections = [-45.0 * pi/180, +45.0 * pi/180]
    ...
    def move(self, motion):
        ...
        if self._braitenberg:
            ''' Insertion of Braitenberg-behaviour start '''
            # Get sensor distances and corresponding sensor directions
            sensorDist = np.asarray(self.sense())
            sensorDirections = self.getSensorDirections()

            for index, d in enumerate(sensorDist):
                if d is not None:
                    # Scale distance to [0...1] anti-proportional
                    d = 1.0 - d/self._maxSenseValue
                else:
            ...
            # Set speed
            v = 2*self._maxSpeed - sensorDist[0] - sensorDist[1]
            ...
            # Set omega
            omega = self._maxOmega * (sensorDist[0] - sensorDist[1])

            ''' Insertion of Braitenberg-behaviour stop '''
