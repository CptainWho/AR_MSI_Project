...
class CarrotDonkey:
    ...
    def set_carrot_position(self, p):
        self.carrot_pos = p
        self.world.drawCircle(p, color="orange")
    ...
    def robot_too_close_to_carrot(self):
        """
        checks if robot maybe is closer to carrot than specified space
        :return:
        """
        dist = Calc.get_dist_from_point_to_point(self.robot_loc.get_robot_point(), self.carrot_pos)
        if dist < self.space:
            return True
        else:
            return False

    def move_carrot_with_donkey(self):
        old_carrot_pos = self.get_carrot_position()
        rel_point = Calc.get_orthogonal_point_on_line(self.carrot_pos, self.p_next, self.robot_loc.get_robot_point(), self.space*1.2)
        new_carrot_pos = [self.carrot_pos[0] + rel_point[0], self.carrot_pos[1] + rel_point[1]]

        dist_new = Calc.get_dist_from_point_to_point(new_carrot_pos, self.p_next)
        dist_abs = Calc.get_dist_from_point_to_point(self.p_last, self.p_next)

        if dist_new > dist_abs:
            self.set_carrot_position(self.p_next)
    ...
    def next_movement_commands(self):
        ...
        robot_close_to_line = self.robot_close_to_line()

        carrot_point = self.get_carrot_position()

        # when robot is closer to target point, than carrot itself, set carrot to projection of robot on the line
        if self.robot_closer_than_carrot(self.p_next, carrot_point):
            # when robot is still close to carrot, add offset
            if robot_close_to_line:
                offset = self.space*2
            else:
                offset = 0
            # Calculate new point for carrot
            rel_point = Calc.get_orthogonal_point_on_line(self.carrot_pos, self.p_next, self.robot_loc.get_robot_point(), offset)
            new_carrot_pos = [self.carrot_pos[0] + rel_point[0], self.carrot_pos[1] + rel_point[1]]

            # when robot still is closer set carrot on point, else update pos
            if self.robot_closer_than_carrot(self.p_next, new_carrot_pos):
                self.set_carrot_position(self.p_next)
                self.update_target_point()
            else:
                self.set_carrot_position(new_carrot_pos)
